...
int CheckChetvert(int x, int y) // ввод ф-ции, определяющей координаты точки
    {
        int res; // ввод имени ф-ции для вывода результата (номер четверти плоскости)
        if (x>0 && y>0) res=1; // 
...
try
    {
        ...
        else Console.WriteLine("Точка находится на какой-то оси");
    }
catch
    {
